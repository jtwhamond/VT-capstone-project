...
```{r}
#continue VARX Model
va.varx <- VAR(y=Y.train, p = 3, exogen = exog.train, type = "const")
summary(va.varx)
```
...
```{r}
#VAR-X with reduced Exogenous variables
exog.train.2 <- exog.train[, !names(exog.train) %in% c("Jul_prec2", "Jul_sp01", "Jun_sp01.diff....diff.X.Jun_sp01.","Jul_prec", "Aug_temp.diff....diff.X.Aug_temp.")]
va.varx2 <- VAR(y=Y.train, p = 3, exogen = exog.train.2, type = "const")
summary(va.varx2)
```
```{r}
#Plot VARX FIRST MODEL
va.varx.fitted <- fitted(va.varx)
va.varx.yld.fitted <- ts(va.varx.fitted[,1], start = 1984, end=2014, frequency =1)
...
acc1 <- c("MSE"=varx.MSE,"RMSE"=varx.RMSE,"MAPE"=varx.MAPE)
knitr::kable(cbind('VARX'=acc1),
...
url2 <- "https://raw.githubusercontent.com/jtwhamond/VT-capstone-project/refs/heads/main/10Y%20Rolling%20Average%20Weather%20Variables%20For%2010%20Year%20Forecast.csv?token=GHSAT0AAAAAADH5GBJWKXG5MDPMI4EYPUBS2EG2N3Q"
