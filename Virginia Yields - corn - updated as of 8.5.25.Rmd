...
```{r}
#DATASET IMPORT
url <- "https://raw.githubusercontent.com/jtwhamond/VT-capstone-project/refs/heads/main/CapstoneData.csv?token=GHSAT0AAAAAADGUGLEL2JFNY5EUDZWTXFVS2ESE4NA"
...
url2 <- "https://raw.githubusercontent.com/jtwhamond/VT-capstone-project/refs/heads/main/10Y%20Rolling%20Average%20Weather%20Variables%20For%2010%20Year%20Forecast.csv?token=GHSAT0AAAAAADGUGLELSOOY4IU2E5DKFNNK2ESE62A"
...
url3 <- "https://raw.githubusercontent.com/jtwhamond/VT-capstone-project/refs/heads/main/County%20Level%20Weather%20Data%20CSV.csv?token=GHSAT0AAAAAADGUGLEKSVLBLAF26CPYDP5W2ESFIUQ"
...
county_weather_metrics <- weather_clean %>%
  ...
  summarise(
    ...
    total_hot_days = sum(tmax_F >= 95.26, na.rm = TRUE),
    avg_extreme_temp = mean(tmax_F[tmax_F >= 94], na.rm = TRUE),
    mod_temp_days = sum(tmax_F < 95.26, na.rm = TRUE),
    extreme_temp_days = sum(tmax_F >= 95.26, na.rm = TRUE),
    .groups = "drop"
  )
```

```{r}
url4 <- "https://raw.githubusercontent.com/jtwhamond/VT-capstone-project/refs/heads/main/County%20Weights%20CSV.csv?token=GHSAT0AAAAAADGUGLEL7CFYZVG2MIEKIU2Y2ESFJQA"
...
lr.train <- lr.train[, !duplicated(names(lr.train))]
...
```{r}
lr.train <- left_join(lr.train, weather_state_weighted %>% rename(YEAR = year), by = "YEAR")
```
