...
```{r}
# Install packages
install.packages(c("lubridate", "mgcv", "tidyverse", "rnassqs"))
```
...
weather_clean <- weather_raw %>%
  ...
  filter(month %in% 4:8)                             # April–August window
```

```{r}
library(broom)
library(gt)
# 1. Count days above 29°C (≈ 84.2°F) for April–August
heat_day_counts <- weather_clean %>%
  filter(month %in% 4:8) %>%
  group_by(county, year, month) %>%
  summarise(days_above_29C = sum(tmax_F > 84.2, na.rm = TRUE), .groups = "drop")

# 2. Bin Tmax values from 82.4°F to 93.2°F (~28°C to 34°C)
...
# 🚨 Check to make sure we found bin variables
if (length(bin_vars) == 0) {
  stop("No bin columns found — check your pivot_wider or column name prefix.")
}
...
gam_summary <- gam_summary %>%
  mutate(
    ...
    term = str_replace_all(term, "\\.", "–"),               # Replace . with dash
    term = str_replace(term, "bin_NA", "Bin NA"),
    term = str_replace_all(term, "bin_91–4–93–2", "91.4–93.2°F"),  # example relabel
...
bin_labels <- c(
  "(82.4–84.2°F)", "(84.2–86°F)", "(86–87.8°F)",
  "(87.8–89.6°F)", "(89.6–91.4°F)", "(91.4–93.2°F)"
)
...
ggplot(monthly_heat_days, aes(x = year, y = hot_days, color = factor(month))) +
  ...
  scale_color_manual(values = RColorBrewer::brewer.pal(5, "Set1"),
                     labels = month_labels) +
  labs(
    title = "Trend in Extreme Heat Days (Tmax > 84.2°F) by Month",
    x = "Year",
    y = "Number of Days",
    color = "Month"
  ) +
...
ggplot(monthly_extreme_days, aes(x = year, y = extreme_days, color = factor(month))) +
  ...
  labs(
    title = "Trend in Extreme Heat Days (Tmax ≥ 91.4°F) by Month",
    x = "Year",
    y = "Number of Days ≥ 91.4°F",
    color = "Month"
  ) +
  theme_minimal(base_size = 14)
```
...
ggplot(jul_aug_extreme, aes(x = year, y = extreme_days, color = factor(month))) +
  ...
  labs(
    title = "Trend in Extreme Heat Days (≥ 91.4°F) in July & August",
    x = "Year",
    y = "Number of Days ≥ 91.4°F",
    color = "Month"
  ) +
